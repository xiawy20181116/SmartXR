...
$WorkDir = Join-Path -Path $RepoRoot -ChildPath ".tmp\script_only_websocket_probe"
...
$GodotLog = Join-Path -Path $WorkDir -ChildPath "godot_websocket_probe.log"
$GodotErr = Join-Path -Path $WorkDir -ChildPath "godot_websocket_probe.err.log"
$StatusFile = Join-Path -Path $WorkDir -ChildPath "script_only_websocket_probe_status.json"
$Publisher = Join-Path -Path $RepoRoot -ChildPath "tools\fake_proxy_targets_publisher.py"
$ProbeScript = Join-Path -Path $RepoRoot -ChildPath "godot-android\tests\script_only_websocket_probe.gd"
...
if (-not (Test-Path -LiteralPath $ProbeScript)) {
    throw "Godot WebSocket probe script not found: $ProbeScript"
}
...
$OldStatusRes = $env:PROXY_TARGETS_WS_STATUS_RES
$OldTimeout = $env:PROXY_TARGETS_WS_TIMEOUT_SEC
$PublisherProcess = $null
$GodotProcess = $null
$ExitCode = 1

Write-Host "SmartXR Godot script-only WebSocketPeer probe"
...
try {
    ...
    $env:PROXY_TARGETS_WS_STATUS_RES = ([System.IO.Path]::GetFullPath($StatusFile)).Replace("\", "/")
    $env:PROXY_TARGETS_WS_TIMEOUT_SEC = [string]$TimeoutSeconds
    ...
    if (-not $GodotProcess.WaitForExit($WaitMs)) {
        Write-Host "Godot WebSocket probe timed out after $ProcessTimeoutSeconds seconds; killing process $($GodotProcess.Id)."
    ...
    if (Test-Path -LiteralPath $StatusFile) {
        ...
        if ([int]$Status.packets -gt 0 -and [int]$Status.connect_error -eq 0) {
            $ExitCode = 0
    ...
    if ($null -eq $OldStatusRes) {
        Remove-Item Env:\PROXY_TARGETS_WS_STATUS_RES -ErrorAction SilentlyContinue
    } else {
        $env:PROXY_TARGETS_WS_STATUS_RES = $OldStatusRes
    }

    if ($null -eq $OldTimeout) {
        Remove-Item Env:\PROXY_TARGETS_WS_TIMEOUT_SEC -ErrorAction SilentlyContinue
    } else {
        $env:PROXY_TARGETS_WS_TIMEOUT_SEC = $OldTimeout
    }
