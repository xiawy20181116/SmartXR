...
class GodotAndroidMeshCardTests(unittest.TestCase):
    ...
    def test_proxy_targets_live_websocket_consumer_is_wired(self):
        ...
        self.assertIn('"last_proxy_position_3d": _vec3_to_status_array(_proxy_targets_last_position)', source)
        ...
        self.assertIn('"card_attach_position_3d": _proxy_targets_card_attach_position_3d()', source)
        self.assertIn('"card_node_position": _proxy_targets_card_node_position()', source)
        self.assertIn('"card_node_position_3d": _proxy_targets_card_node_position_3d()', source)
        ...
        self.assertIn("func _proxy_targets_card_attach_position_3d() -> Array:", source)
        self.assertIn("func _proxy_targets_card_node_position() -> String:", source)
        self.assertIn("func _proxy_targets_card_node_position_3d() -> Array:", source)
        self.assertIn("func _vec3_to_status_array(value: Vector3) -> Array:", source)
        ...
        self.assertIn("ProxyWS: %s sub=%s packets=%d parsed=%d live=%d apply=%d seq=%d bytes=%d type=%s pos=%s attach=%s card=%s local=%s world=%s src=%s err=%s", source)
    ...
    def test_proxy_targets_status_reports_head_to_world_diagnostics(self):
        ...
        self.assertIn('"proxy_local_position_3d": _vec3_to_status_array(_proxy_targets_last_local_position)', source)
        self.assertIn('"proxy_world_position": _format_vec3(_proxy_targets_last_world_position)', source)
        self.assertIn('"proxy_world_position_3d": _vec3_to_status_array(_proxy_targets_last_world_position)', source)

    def test_proxy_targets_status_label_shows_attach_coordinate_chain(self):
        source = SCRIPT.read_text(encoding="utf-8")

        self.assertIn("attach=%s", source)
        self.assertIn("_proxy_targets_card_resolved_position()", source)
        self.assertIn("local=%s", source)
        self.assertIn("_format_vec3(_proxy_targets_last_local_position)", source)
        self.assertIn("world=%s", source)
        self.assertIn("_format_vec3(_proxy_targets_last_world_position)", source)
