...
class GodotProxyTargetsStatusFragmentTests(unittest.TestCase):
    def test_fragment_is_dependency_free_diagnostics_store(self):
        ...
        self.assertIn("var _last_runtime_local_position := Vector3.ZERO", source)
        self.assertIn('var _last_head_z_mode := "negative_z_forward"', source)
        self.assertIn('var _last_anchor_mode := "dynamic"', source)
        self.assertIn("var _last_world_latched := false", source)
        self.assertIn('var _last_world_latch_state := "-"', source)
        ...
        self.assertIn('"card_reacquire_state": str(runtime_values.get("card_reacquire_state", "idle"))', source)
    ...
    def test_moving_card_delegates_proxy_diagnostics_to_fragment(self):
        ...
        self.assertIn('"card_reacquire_state": _card_attachment.reacquire_state(CARD_ANCHOR_NAME)', card)
    ...
    def test_runtime_probe_runner_and_ci_docs_exist(self):
        ...
        self.assertIn("records_head_z_mode", probe)
        self.assertIn("records_world_latch_info", probe)
        ...
        self.assertIn("status_values_includes_card_reacquire_state", probe)
