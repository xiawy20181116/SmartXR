...
class GodotCardAttachmentTests(unittest.TestCase):
    def test_card_attachment_owns_store_fallbacks_and_offset_math(self):
        ...
        self.assertIn("func update_attachments(card_anchor: Node3D, primary_card_id: String, delta_s := 0.0) -> bool:", source)
        ...
        # Dynamic fresh re-acquire smoothing/clamping sits here because this
        # subsystem is the one that owns card-world transform application.
        self.assertIn("func set_reacquire_rule(reacquire_rule: Dictionary) -> void:", source)
        self.assertIn("func reacquire_state(card_id: String) -> String:", source)
        self.assertIn('const REACQUIRE_STATE_IDLE := "idle"', source)
        self.assertIn('const REACQUIRE_STATE_SMOOTHING := "smoothing"', source)
        self.assertIn('const REACQUIRE_STATE_CLAMPED := "clamped"', source)
        self.assertIn('const REACQUIRE_STATE_SETTLED := "settled"', source)
        self.assertIn('str(_reacquire_rule.get("trigger", "dynamic_held_to_dynamic"))', source)
        self.assertIn('attachment["reacquire_state"] = REACQUIRE_STATE_CLAMPED', source)
        ...
        self.assertIn("static func offset_transform_with_context(", source)
        self.assertIn("static func depth_scaled_right_half_width_transform(", source)
        self.assertIn("static func depth_scaled_right_angle_transform(", source)
        self.assertIn('"depth_offset_m"', source)
        self.assertIn('"right_angle_deg"', source)
        self.assertIn("static func _horizontal_reference_right_axis(", source)
        self.assertIn("static func offset_vector(offset_rule) -> Vector3:", source)
        self.assertIn('"depth_scaled_right_half_width":', source)
        ...
        self.assertIn("func set_reference_transform_provider(reference_transform_provider: Callable) -> void:", source)
    ...
    def test_moving_card_delegates_to_extracted_attachment_subsystem(self):
        ...
        self.assertIn("_card_attachment.set_reacquire_rule(_options.proxy_targets_card_reacquire_rule(PROXY_TARGETS_CARD_REACQUIRE_RULE))", card)
        ...
        self.assertIn("if not _card_attachment.update_attachments(_card_anchor, CARD_ANCHOR_NAME, delta_s):", card)
    ...
    def test_runtime_probe_and_runner_exist(self):
        ...
        self.assertIn("metric_offset_scales_depth_and_right_half_width", probe)
        ...
        self.assertIn("dynamic_reacquire_clamps_first_fresh_step", probe)
        self.assertIn("dynamic_reacquire_reports_clamped_state", probe)
        self.assertIn("dynamic_reacquire_settles_after_catching_target", probe)
        self.assertIn("dynamic_reacquire_reports_settled_state", probe)
